...
ARTICLE_LINKS="$(
  (
    ...
    regex='<category>.*\b(Newsletter|Update)\b.*</category>'
    page=1
    found=0
    while [ $found -eq 0 ]; do
      ...
      [[ $content =~ $regex ]] && found=1
      ((page++))
      [ $page -gt 10 ] && break
    done
    echo '</channel></rss>'
  ) | \
    xee xpath "string-join(//item[category = ('Newsletter', 'Update')][1]/preceding-sibling::item[not(category = ('Veranstaltungen'))]/link, ' ')" | \
    tr -d '"'
)"
